...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def logprob(self, weight, bias, hiddens, splits=None, softmaxed_head_res=None, verbose=False):
        ...
        for idx in splits:
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                results.append(head_entropy.reshape(-1, 1) + tail_entropy)
    ...
    def split_on_targets(self, hiddens, targets):
        ...
        #masks = [(targets >= self.splits[idx]).reshape(1, -1) for idx in range(1, self.nsplits)]
        ...
        for idx in range(self.nsplits):
            ...
            split_hiddens.append(hiddens.masked_select(tmp_mask.unsqueeze(1).expand_as(hiddens)).reshape(-1, hiddens.size(1)))
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        total_loss = None
        if len(hiddens.size()) > 2: hiddens = hiddens.reshape(-1, hiddens.size(2))
        ...
        for idx in range(self.nsplits):
            ...
            # For those targets in the head (idx == 0) we only need to return their loss
            if idx == 0:
                entropy = -torch.gather(softmaxed_head_res, dim=1, index=split_targets[idx].reshape(-1, 1))
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
            else:
                if self.verbose or verbose:
                    start, end = self.splits[idx], self.splits[idx + 1]
                ...
                indices = (split_targets[idx] - self.splits[idx]).reshape(-1, 1)
            ...
            total_loss = total_loss + entropy.float().sum()
